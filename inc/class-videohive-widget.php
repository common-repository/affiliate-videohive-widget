...
/*	VideoHive Widget Class */

class PB_VideoHive_Widget extends WP_Widget {

	var $vh_cats; //VideoHive items categories
	var $exclude; //Wheter to exclude items or not
	var $defaults;

	function __construct() {
		$widget_ops = array( 'classname' => 'pb_videohive_widget', 'description' => __( 'Display VideoHive items with this widget', 'affiliate-videohive-widget' ) );
		$control_ops = array( 'id_base' => 'pb_videohive_widget' );
		parent::__construct( 'pb_videohive_widget', __( 'Affiliate VideoHive Widget', 'affiliate-videohive-widget' ), $widget_ops, $control_ops );

		$this->vh_cats = array(
			array( 'name' => 'after-effects-project-files', 'title' => 'After Effects' ),
			array( 'name' => 'premiere-pro-templates', 'title' => 'Premiere Pro' ),
			array( 'name' => 'apple-motion-templates', 'title' => 'Apple Motion' ),
			array( 'name' => 'motion-graphics', 'title' => 'Motion Graphics' ),
			array( 'name' => 'stock-footage', 'title' => 'Stock Footage' ),
			array( 'name' => 'add-ons', 'title' => 'Add Ons' ),
			array( 'name' => 'cinema-4d-templates', 'title' => 'Cinema 4D' ),
		);
		...
		$this->defaults = array(
			'title' => 'VideoHive',
			'description' => '',
			'items_type' => array( 'premiere-pro-templates' ),
			'items_from' => 'user',
			'user' => 'replayer',
			'num_items' => 9,
			'orderby' => 'uploaded_on',
			'ref' => 'pluginsbay',
			'more_link_url' => 'https://videohive.net/user/replayer/portfolio?ref=pluginsbay',
			'more_link_txt' => __( 'View more', 'affiliate-videohive-widget' ),
			'order' => 'desc',
			'target' => '_blank',
			'exclude' => '',
			'nofollow' => 0,
		...
		$this->defaults = apply_filters( 'pb_vh_widget_modify_defaults', $this->defaults );
...
?>
		<ul class="pb_videohive_widget_ul">
		...
		<?php if ( !empty( $instance['more_link_url'] ) ): ?>
			 <?php $more_text = isset( $instance['more_link_txt'] ) && !empty( $instance['more_link_txt'] ) ? $instance['more_link_txt'] : __( 'View more', 'affiliate-videohive-widget' ); ?>
	...
	function form( $instance ) {
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'title' ); ?>">
		        <?php _e( 'Title', 'affiliate-videohive-widget' ); ?>:</label>
		    <input id="<?php echo $this->get_field_id( 'title' ); ?>" type="text" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" class="widefat" />
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'description' ); ?>">
		        <?php _e( 'Description', 'affiliate-videohive-widget' ); ?>:</label>
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'token' ); ?>">
		        <?php _e( 'Envato API token', 'affiliate-videohive-widget' ); ?>:</label>
		    ...
		    <small class="howto"><?php _e( 'Provide your Envato API token', 'affiliate-videohive-widget' ); ?></small>
		    <small class="howto"><?php echo '<a href="https://build.envato.com/create-token/" target="_blank">'.__( 'Generate token', 'affiliate-videohive-widget' ).'</a>'; ?></small>
		    
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'num_items' ); ?>">
		        <?php _e( 'Number of items to show', 'affiliate-videohive-widget' ); ?>:</label>
		...
		<p>
		    <label>
		        <?php _e( 'Select items from', 'affiliate-videohive-widget' ); ?>:</label><br />
		    <input id="<?php echo $this->get_field_id( 'select_from_latest' ); ?>" type="radio" name="<?php echo $this->get_field_name( 'items_from' ); ?>" value="latest" <?php checked( $instance['items_from'], 'latest' );?> class="affiliate-videohive-widget-type" /> <label for="<?php echo $this->get_field_id( 'select_from_latest' ); ?>">
		        <?php _e( 'Latest Items', 'affiliate-videohive-widget' ); ?></label><br />
		    <input id="<?php echo $this->get_field_id( 'select_from_user' ); ?>" type="radio" name="<?php echo $this->get_field_name( 'items_from' ); ?>" value="user" <?php checked( $instance['items_from'], 'user' );?> class="affiliate-videohive-widget-type" /> <label for="<?php echo $this->get_field_id( 'select_from_user' ); ?>">
		        <?php _e( 'Specific User(s)', 'affiliate-videohive-widget' ); ?></label>
		</p>
		<p class="affiliate-videohive-widget-user" <?php echo $this->selected( $instance['items_from'], 'user', 'style="display:none;"', true ); ?>>
		    <label for="<?php echo $this->get_field_id( 'user' ); ?>">
		        <?php _e( 'VideoHive username(s)', 'affiliate-videohive-widget' ); ?>:</label>
		    ...
		    <small class="howto"><?php _e( 'For multiple users, separate by comma: i.e. user1,user2,user3', 'affiliate-videohive-widget' ); ?></small>
		</p>
		<p class="affiliate-videohive-widget-cat" <?php echo $this->selected( $instance['items_from'], 'popular', 'style="display:none;"'); ?>>
		    <label>
		        <?php _e( 'Item categories to show', 'affiliate-videohive-widget' ); ?>:</label><br />
		    <?php foreach ( $this->vh_cats as $cat ) : ?>
		...
		<p class="affiliate-videohive-widget-order" <?php echo $this->selected( $instance['items_from'], 'user', 'style="display:none;"', true ); ?>>
		    <label>
		        <?php _e( 'Order by', 'affiliate-videohive-widget' ); ?>:</label>
		...
		<p class="affiliate-videohive-widget-order" <?php echo $this->selected( $instance['items_from'], 'user', 'style="display:none;"', true ); ?>>
		    <input id="<?php echo $this->get_field_id( 'order_asc' ); ?>" type="radio" name="<?php echo $this->get_field_name( 'order' ); ?>" value="asc" <?php checked( $instance['order'], 'asc' );?> /> <label for="<?php echo $this->get_field_id( 'order_asc' ); ?>">
		        <?php _e( 'Ascending', 'affiliate-videohive-widget' ); ?></label>
		    <input id="<?php echo $this->get_field_id( 'order_desc' ); ?>" type="radio" name="<?php echo $this->get_field_name( 'order' ); ?>" value="desc" <?php checked( $instance['order'], 'desc' );?> /> <label for="<?php echo $this->get_field_id( 'order_desc' ); ?>">
		        <?php _e( 'Descending', 'affiliate-videohive-widget' ); ?></label>
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'exclude' ); ?>">
		        <?php _e( 'Exclude item(s)', 'affiliate-videohive-widget' ); ?>:</label>
		    ...
		    <small class="howto"><?php _e( 'Specify item ID to exclude specific item (separate by comma for multiple items): i.e. 8134834,7184572', 'affiliate-videohive-widget' ); ?></small>
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'ref' ); ?>">
		        <?php _e( 'Referral user', 'affiliate-videohive-widget' ); ?>:</label>
		    ...
		    <small class="howto"><?php _e( 'Specify username if you want to use items as VideoHive affiliate links', 'affiliate-videohive-widget' ); ?></small>
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'more_link_url' ); ?>">
		        <?php _e( 'More link URL', 'affiliate-videohive-widget' ); ?>:</label>
		    ...
		    <small class="howto"><?php _e( 'Specify URL if you want to show "more" link under the items list', 'affiliate-videohive-widget' ); ?></small>
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'more_link_txt' ); ?>">
		        <?php _e( 'More link text', 'affiliate-videohive-widget' ); ?>:</label>
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'target' ); ?>">
		        <?php _e( 'Open items in', 'affiliate-videohive-widget' ); ?>: </label>
		    <select id="<?php echo $this->get_field_id( 'target' ); ?>" name="<?php echo $this->get_field_name( 'target' ); ?>">
		        <option value="_blank" <?php selected( '_blank' , $instance['target'] ); ?>>
		            <?php _e( 'New Window', 'affiliate-videohive-widget' ); ?>
		        </option>
		        <option value="_self" <?php selected( '_self' , $instance['target'] ); ?>>
		            <?php _e( 'Same Window', 'affiliate-videohive-widget' ); ?>
		        </option>
		    </select>
		</p>
		<p>
		    <label for="<?php echo $this->get_field_id( 'nofollow' ); ?>"><input id="<?php echo $this->get_field_id( 'nofollow' ); ?>" type="checkbox" name="<?php echo $this->get_field_name( 'nofollow' ); ?>" value="1" <?php checked( $instance['nofollow'], 1 ); ?> class="widefat" />
		        <?php _e( 'Add rel="nofollow" to item links', 'affiliate-videohive-widget' ); ?>
		    </label>
		</p>
	...
	function get_items_from_users( $users = array(), $type = array( 'wordpress' ) ) {
		...
		foreach ( $users as $user ) {
			...
			if ( empty( $cached ) ) {
				...
				$query = 'new-files-from-user:'.$user.',videohive.json';
			...
			$type_check = count( $type ) == count( $this->vh_cats ) ? false : true;
	...
	function get_latest_items( $types = array( 'wordpress' ) ) {
		...
		foreach ( $types as $type ) {
			...
			if ( empty( $cached ) ) {
				...
				$query = 'new-files:videohive,'.$type.'.json';
	...
	function enqueue_styles() {
		wp_register_style( 'pluginsbay-videohive-widget', PB_VH_PLUGIN_URI . 'css/style.css', false, PB_VH_WIDGET_VER );
		wp_enqueue_style( 'pluginsbay-videohive-widget' );
	}
	...
	function enqueue_scripts() {
		...
		if( $pagenow == 'widgets.php' ){
			wp_enqueue_script( 'pluginsbay-videohive-widget', PB_VH_PLUGIN_URI . 'js/admin.js', array('jquery') , PB_VH_WIDGET_VER );
		}	
	}
